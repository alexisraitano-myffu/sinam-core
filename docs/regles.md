...
| N1-d | Pendant la lecture des raisons de garder. | L'auteur PREND POSITION : un jugement, une préférence, un changement d'avis, une opinion sur quelqu'un ou quelque chose. | Garder la capture et descendre au routage. Ne pas compter comme prise de position une correction de soi (« en fait je me suis trompé ») ni une nuance de certitude (« il déménage probablement ») : la première parle d'une croyance passée, la seconde du degré de sûreté d'un fait. | garantie · exemples |
...
| # | QUAND | SI | ALORS | Étiquettes |
|---|---|---|---|---|
| N4-a | Au nœud « reste-t-il quelque chose à faire », une fois le projet écarté. | La capture énonce une action encore à faire, sous n'importe quelle forme : infinitif, impératif, deuxième personne, « il faut que je », « je dois ». Deux mots suffisent. | Produire un souvenir de nature `task`, puis passer au nœud de la micro-course avant de l'émettre. | garantie · exemples |
| N4-b | Au nœud « reste-t-il quelque chose à faire ». | La capture ANNULE une action au lieu d'en demander une (« j'annule la réunion de demain », « finalement je n'appelle pas le dentiste »). | Ne produire aucune tâche, si actif que soit le verbe. Descendre au nœud des notes, où la décision d'annuler devient une note, et renseigner le champ d'annulation (voir N9-f). | garantie · exemples |
...
| N6-d | Au nœud de l'occurrence datée, quand la capture parle d'un anniversaire. | Trois formulations possibles, à distinguer. | **Une CÉLÉBRATION nommée** (fête, apéro, dîner) → un `event` à la date de la CÉLÉBRATION, récurrence à FAUX : c'est la fête qui est datée, et rien ne dit qu'elle se rejouera le même jour. **Une date d'anniversaire NUE**, ou mentionnée après coup sans mot de fête → un `event` quand même, récurrence à FAUX, et la confiance sous le seuil pour que l'utilisateur tranche (c'est `N6-f`). Ne JAMAIS retirer la note : le jour où l'on s'est retrouvés n'est pas forcément le jour de la naissance, et seule une confiance basse pose la question. Ce qui revient chaque année n'est pas cet événement mais le FAIT `has_birthday` sur la fiche de la personne (voir `G4-e`) : la récurrence est portée là, jamais par une occurrence au calendrier. **Une NAISSANCE énoncée** → aucun souvenir, même raison. | garantie · prompt |
...
| N8-e | Au nœud de la note. | La capture cite une œuvre, un auteur, une idée extérieure, et l'auteur prend position dessus. | Produire une note. | garantie · prompt |
...
| # | QUAND | SI | ALORS | Étiquettes |
|---|---|---|---|---|
| N10-a | En dernier, une fois tout le reste décidé. | On s'apprête à rendre TROIS souvenirs ou plus. | Ne pas les rendre tels quels : repasser chacun au test de `N10-b`, et fusionner tous ceux qui n'y répondent pas. Si trois ou plus survivent au test, les rendre ET descendre la confiance sous le seuil : au-delà de deux, la capture a probablement été hachée. | préférence · prompt |
...
| G4-e | Quand la capture parle de la date de naissance de quelqu'un. | Il faut décider si l'on émet has_birthday et avec quelle certitude. | Regarder D'ABORD la date elle-même : une date seulement déduite d'un repère (« demain », « mardi », « ce week-end ») n'est JAMAIS explicite, si clairement que la phrase énonce l'anniversaire ; la résoudre et l'émettre en implicite. Puis trois cas. **Date ÉCRITE**, un jour avec son mois → has_birthday, explicite. **Date lue sur une FÊTE**, la capture portant le mot anniversaire → has_birthday quand même, implicite, pour qu'elle parte en validation. **Aucun mot d'anniversaire, ou un ÂGE à la place d'une date** → pas de has_birthday du tout ; l'âge garde son propre fait. | garantie · prompt |
...
| G5-d | Quand la capture déclare qu'une chose s'appelle désormais autrement. | La déclaration est explicite (« mon projet ne s'appelle plus X mais Y », « Acme a été renommé Globex »). | Émettre une PROPOSITION de renommage et laisser le nom actuel intact. Ne jamais écrire le nouveau nom soi-même, ne jamais émettre en plus un fait pour le renommage, ne jamais lire un renommage dans une simple variante d'orthographe ou un surnom : ceux-là sont des alias. | garantie · code |
