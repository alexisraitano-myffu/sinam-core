...
- **Quatre décisions d'identité ne s'appliquent JAMAIS à la capture** — type d'entité (SYN-58), fusion d'entités (SYN-39/61), fusion de prédicats (SYN-190), renommage d'entité (SYN-188). Elles s'écrivent en propositions (`*_proposals`, toutes répliquées) et un humain tranche. L'invariant n'est pas « le modèle se trompe parfois » mais **ce que l'erreur coûte** : chacune change l'IDENTITÉ de quelque chose que l'utilisateur lit comme étant sa mémoire, et une identité changée à tort ne se remarque pas. La négation d'un fait (SYN-189) est le seul cas qui peut s'appliquer tout de suite, et c'est la même règle appliquée : périmer est RÉVERSIBLE (`obsoleted_at`, annulé par `/fact/{id}/restore`). Quand sa cible n'est pas certaine, elle retourne en file comme les autres.
